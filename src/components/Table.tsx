...
const columns = columnDraft.map((col)=>{
    if(col.field==='col1'){
        return col
    }
    return {
...
export const Table:FC<TableProps>=({userRequests})=>{
    ...
    return(
        <ThemeProvider theme={theme}>
            <DataGrid style={{backgroundColor:applicationTheme==='light'?'white':'#152238',fontSize:'1rem',}} pageSizeOptions={[5]} disableRowSelectionOnClick initialState={{
                pagination:{
                    paginationModel:{
                        pageSize:5,
                    }
